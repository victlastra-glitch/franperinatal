...
spreadsheet = {
  ...
  insertSheet: (name) => {
    if (name === 'notification_outbox') return outboxSheet;
    return sheet;
  },
};
  ...
  UrlFetchApp: {
    fetch: (url) => {
      ...
      if (String(url).includes('/payment/create')) {
        ...
        return {
          getResponseCode: () => 200,
          getContentText: () => JSON.stringify({ url: 'https://www.flow.cl/app/web/pay', token: 'FLOWTOKENOPAQUE1234567890ABCD' }),
        };
      }
      ...
      if (String(url).includes('/refund/create')) {
        return {
          getResponseCode: () => 200,
          getContentText: () => JSON.stringify({ token: 'REFUNDTOKENOPAQUE1234567890ABCD', status: 'created' }),
        };
      }
      if (String(url).includes('/refund/getStatus')) {
        return {
          getResponseCode: () => 200,
          getContentText: () => JSON.stringify({ token: 'REFUNDTOKENOPAQUE1234567890ABCD', status: 'accepted' }),
        };
      }
...
const idempotencyKey = 'fran-booking-bbbbbbbb-e89b-12d3-a456-4266141740bb';
...
check(cancel.ok && record().booking_status === 'cancelled' && record().schedule_status === 'cancelled',
  'patient cancel releases the slot immediately under BUSINESS_POLICY_TBD');
...
check(record().payment_status === 'paid' && record().patient_reschedule_count === '1'
  ...
  'cancel keeps historical payment, quota=1, and does not call Flow refund');
check(String(record().notification_outbox_key || '').includes('SESSION_CANCELLED')
  || outboxRows.some((row) => row.reservation_id === record().reservation_id && row.event_type === 'SESSION_CANCELLED'),
  'cancellation queues SESSION_CANCELLED with TBD copy, not PATIENT_CANCELLED refund-success');

const cancelKey = String(outboxRows.find((row) => row.reservation_id === record().reservation_id
  && row.event_type === 'SESSION_CANCELLED').logical_key);
...
check(/Si corresponde un reembolso, te contactaremos/.test(cancelMail.body)
  ...
  'cancellation uses TBD copy, no Meet, no Reagendar, and no Cancelar');
mailBodies = [];
...
const poison = {
  rowNumber: 99,
  reservation_id: 'fran-booking-reservation-poison',
  idempotency_key: 'fran-booking-poison-key-001',
  ...
  notification_outbox_key: 'lifecycle_fran-booking-reservation-poison_BOOKING_CONFIRMED_1',
...
check(poisonRun.ok && poisonRun.results[0].ok && poison.notification_patient_state === 'sent'
  ...
  && mailBodies[0].subject.startsWith('Tu sesión fue reagendada') && !mailBodies[0].body.includes('Reagendar:'),
...
check(record().booking_status === 'cancelled' && record().schedule_status === 'cancelled'
  ...
  'terminal booking remains cancelled with historical payment under BUSINESS_POLICY_TBD');
