...
const sources = await Promise.all(['../Code.js', '../Lifecycle.js', '../EmailTemplates.js', '../CalendarGateway.js']
...
const idempotencyKey = 'fran-booking-123e4567-e89b-12d3-a456-426614174000';
const payload = {
  action: 'create_flow_payment', idempotencyKey, serviceType: 'initial', modality: 'online',
  date: '2026-08-27', time: '11:00', name: 'Synthetic', email: allowlisted, phone: '', patientRut: '11.111.111-1', address: 'Calle Sintetica 123', comuna: 'Providencia', reason: '', message: '',
};
...
check(flow.validCommerceOrder_(order), 'commerceOrder matches fp-<40hex> contract');
check(order.startsWith('fp-') && !order.startsWith('npo-'), 'commerce order uses production prefix');

// 4-10. signed create request shape
fetchImpl = (url, options) => {
  check(url === 'https://www.flow.cl/api/payment/create', 'Flow create uses production /payment/create');
  ...
  check(params.amount === String(flow.INITIAL_PRICE_CLP) && params.currency === 'CLP'
    && Number(params.amount) === 50000, 'amount/currency representation uses canonical 50000 CLP');
  check(params.commerceOrder === order, 'commerceOrder in body');
  check(params.subject === 'Sesión Francisca Bustos', 'subject present');
...
check(created.ok && created.paymentUrl.startsWith('https://www.flow.cl/app/web/pay?token=')
...
}, 'synthetic-flow-secret'), 'request signature includes the 50000 amount correctly');
check(flow.consultationAmountClp_() === 50000, 'refund amount derives from canonical paid amount');
const status = flow.paymentStatus_({ parameter: { st: created.publicStatusToken } });
check(status.ok && status.amount === 50000 && status.currency === 'CLP', 'public payment status returns amount 50000');
...
const failed = context.createFlowPayment_({
  postData: { contents: JSON.stringify({ ...payload, idempotencyKey: 'fran-booking-123e4567-e89b-12d3-a456-426614174001' }) },
});
...
const badJson = context.createFlowPayment_({
  postData: { contents: JSON.stringify({ ...payload, idempotencyKey: 'fran-booking-123e4567-e89b-12d3-a456-426614174002' }) },
});
...
const network = context.createFlowPayment_({
  postData: { contents: JSON.stringify({ ...payload, idempotencyKey: 'fran-booking-123e4567-e89b-12d3-a456-426614174003' }) },
});
...
const abandoned = flow.abandonFailedCheckout_(rows[0].reservation_id);
...
assert.throws(() => flow.abandonFailedCheckout_(rows[0].reservation_id), /BOOKING_NOT_RETRYABLE/);
assertions += 1;
