...
const baseProperties = {
  ...
  FLOW_CONFIRMATION_URL: 'https://franciscabustos.cl/api/flow-confirmation', BOOKING_STORE_ID: 'synthetic-store', CALENDAR_ID: 'synthetic-calendar',
...
const event = { id: 'event-opaque-1', etag: 'etag-1', updated: '2026-08-24T13:00:00.000Z', status: 'confirmed',
  ...
  extendedProperties: { private: { source: 'fran_booking', link_key: 'fran-booking-calendar-link-abcdef0123456789', schema: 'fran_booking:v1' } },
...
const recordTemplate = { reservation_id: 'fran-booking-reservation-opaque', calendar_event_id: event.id, calendar_link_key: event.extendedProperties.private.link_key,
...
const cancelRecord = { ...txnRecord, reservation_id: 'fran-booking-reservation-cancel', booking_status: 'confirmed', schedule_status: 'scheduled', patient_reschedule_count: '0',
...
check(cancelResult.ok && cancelResult.refund === 'BUSINESS_POLICY_TBD' && cancelReplay.replay === true && cancelRecord.schedule_status === 'cancelled'
  && cancelRecord.booking_status === 'cancelled'
  && cancelNotifications === 2 && cancelRefunds === 0, 'patient cancellation is idempotent, releases capacity, and enqueues one operational manual-review path');
let alertFailures = 0;
const alertFailCap = phase.createCapability_('CANCEL', { secret, now: Date.parse('2026-08-23T12:00:00Z'), expiresAt: '2026-08-24T12:00:00Z' });
const alertFailRecord = { ...txnRecord, reservation_id: 'fran-booking-reservation-alert-fail', booking_status: 'confirmed', schedule_status: 'scheduled',
  payment_status: 'paid', refund_status: 'not_required', ...phase.capabilityFields_(phase.capabilityForStorage_(alertFailCap)) };
const alertFailStore = { loadByReservationId: () => alertFailRecord, update: (_record, fields) => Object.assign(alertFailRecord, fields) };
const alertFailResult = phase.patientCancelTransaction_({ reservationId: alertFailRecord.reservation_id, token: alertFailCap.token, now: fixedNow,
  deps: { ...cancelDeps, store: alertFailStore, enqueueNotification: () => { alertFailures += 1; throw new Error('smtp'); } } });
check(alertFailResult.ok && alertFailResult.refund === 'BUSINESS_POLICY_TBD' && alertFailRecord.booking_status === 'cancelled'
  && alertFailRecord.payment_status === 'paid' && alertFailRecord.refund_status === 'manual_review' && alertFailures === 1,
  'operational alert failure does not corrupt booking, payment, or refund state');

// F. Refund idempotency/callback safety and notification CTA policy.
let refundCalls = 0; const refundRecord = { reservation_id: 'fran-booking-reservation-refund', refund_status: 'refund_requested', refund_commerce_order: '', refund_provider_reference: '', payment_status: 'paid' };
...
const refundFirst = refund.refundCreateOnce_({ store: refundStore, record: refundRecord, gateway: refundGateway, receiverEmail: 'ops@example.test', amount: '500', urlCallBack: 'https://franciscabustos.cl/api/refund-confirmation', commerceTrxId: 'commerce-opaque' });
const refundSecond = refund.refundCreateOnce_({ store: refundStore, record: refundRecord, gateway: refundGateway, receiverEmail: 'ops@example.test', amount: '500', urlCallBack: 'https://franciscabustos.cl/api/refund-confirmation', commerceTrxId: 'commerce-opaque' });
...
let timeoutCalls = 0; const timeoutRecord = { reservation_id: 'fran-booking-reservation-timeout', refund_status: 'refund_requested', refund_commerce_order: '' };
const timeoutResult = refund.refundCreateOnce_({ store: { update: (_record, fields) => Object.assign(timeoutRecord, fields) }, record: timeoutRecord,
  gateway: { create: () => { timeoutCalls += 1; const error = new Error('timeout'); error.code = 'FLOW_TIMEOUT'; throw error; } }, receiverEmail: 'ops@example.test', amount: '500', urlCallBack: 'https://franciscabustos.cl/api/refund-confirmation', commerceTrxId: 'commerce-opaque' });
const timeoutRetry = refund.refundCreateOnce_({ store: { update: (_record, fields) => Object.assign(timeoutRecord, fields) }, record: timeoutRecord,
  gateway: { create: () => { timeoutCalls += 1; return { providerReference: 'must-not-run' }; } }, receiverEmail: 'ops@example.test', amount: '500', urlCallBack: 'https://franciscabustos.cl/api/refund-confirmation', commerceTrxId: 'commerce-opaque' });
...
const rejectedRecord = { reservation_id: 'fran-booking-reservation-rejected', refund_status: 'refund_requested', refund_commerce_order: '' };
...
const transportGateway = refund.createFlowRefundGateway_({
  baseUrl: 'https://www.flow.cl/api', apiKey: 'synthetic-flow-key', secretKey: 'synthetic-flow-secret',
  fetch: (url, options) => {
...
transportGateway.create({ reservationId: 'fran-booking-reservation-transport', receiverEmail: 'ops@example.test', amount: '500',
  urlCallBack: 'https://franciscabustos.cl/api/refund-confirmation', commerceTrxId: 'commerce-opaque' });
...
check(refundTransportCalls.length === 3 && refundTransportCalls.every((call) => call.parsed.hostname === 'www.flow.cl'), 'refund transport is production-only');
...
assert.throws(() => transportGateway.create({ reservationId: 'fran-booking-reservation-invalid-callback', receiverEmail: 'ops@example.test', amount: '500',
...
const pageOneEvent = pageEvent('page-one', 'fran-booking-calendar-link-pageone', '2026-08-24T13:00:00.000Z');
const pageTwoEvent = pageEvent('page-two', 'fran-booking-calendar-link-pagetwo', '2026-08-24T13:00:00.000Z');
const pageRequests = [];
...
const cleanRecord = { reservation_id: 'fran-booking-reservation-clean', calendar_event_id: pageOneEvent.id, calendar_event_etag: 'old', calendar_event_updated_at: '2026-08-24T12:00:00.000Z', calendar_sync_hash: 'old', booking_status: 'confirmed', schedule_status: 'scheduled', patient_reschedule_count: '0', notification_version: '1' };
const staleRecord = { reservation_id: 'fran-booking-reservation-stale', calendar_event_id: pageTwoEvent.id, calendar_event_etag: 'old', calendar_event_updated_at: '2026-08-24T18:00:00.000Z', calendar_sync_hash: 'old', booking_status: 'confirmed', schedule_status: 'scheduled', patient_reschedule_count: '0', notification_version: '1' };
...
const lookupRecord = { reservation_id: 'fran-booking-reservation-different', calendar_link_key: pageOneEvent.extendedProperties.private.link_key,
...
const selfEvent = pageEvent('self-event', 'fran-booking-calendar-link-selfevent', '2026-08-24T13:00:00.000Z');
...
const competitor = pageEvent('competitor', 'fran-booking-calendar-link-competitor', '2026-08-24T13:00:00.000Z');
