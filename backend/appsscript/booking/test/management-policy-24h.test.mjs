...
const CAPABILITY_SECRET = 'synthetic-capability-secret-20260823-abcdefghijklmnopqrstuvwxyz';
const OPS_EMAIL = 'ops@example.test';
...
const PATIENT_EMAIL = 'paciente@example.test';
...
function buildHarness(patches) {
  ...
  return { context, phase, worker, state, setNow, sheet, schema, drain, rowFor, paidBooking, currentRows };
}

// ---------------------------------------------------------------------------
// Discover the real America/Santiago DST transitions rather than hardcoding one.
// ---------------------------------------------------------------------------
function santiagoOffsetMinutes(ms) {
  const parts = {};
...
function findTransition(fromMs, toMs) {
  const minute = 60000;
