...
const sourcePaths = ['../Code.js', '../Lifecycle.js', '../EmailTemplates.js'];
...
check(api.HEADERS.length === 65, 'schema header count is exact');
...
check(api.transitionAllowed_('payment_status', 'pending', 'annulled'), 'Flow status 4 maps pending to unpaid annulled');
check(api.transitionAllowed_('payment_status', 'annulled', 'pending'), 'hold-valid retry may return annulled to pending');
check(!api.transitionAllowed_('payment_status', 'annulled', 'paid'), 'annulled cannot jump to paid without a new pending order');
check(context.refundPolicy_().decision === 'BUSINESS_POLICY_TBD' && context.refundPolicy_().eligible === false,
  'canonical refund policy remains BUSINESS_POLICY_TBD');
check(context.activeRefundPolicy_({ payment_status: 'paid', booking_status: 'confirmed' }).eligible === false,
  'without automatic refund, paid cancellation is not refund-eligible');
...
const record = { idempotency_key: 'fran-booking-123e4567-e89b-42d3-a456-426614174000' };
...
const confirmedRecord = {
  reservation_id: 'fran-booking-reservation-occ',
  idempotency_key: 'fran-booking-123e4567-e89b-42d3-a456-426614174000',
  commerce_order: 'fp-aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
...
check(confirmKey !== api.notificationOccurrenceKey_({ ...confirmedRecord, commerce_order: 'fp-bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb' }, 'BOOKING_CONFIRMED'),
...
for (const eventType of eventTypes) {
  ...
  const base = {
    ...
    refund_commerce_order: 'fp-rf-aaaaaaaaaaaaaaaaaaaaaaaa',
  ...
  const other = api.notificationOccurrenceKey_({
    ...
    commerce_order: eventType === 'BOOKING_CONFIRMED' ? 'fp-cccccccccccccccccccccccccccccccccccccccc' : base.commerce_order,
    calendar_event_etag: 'etag-y',
    calendar_event_updated_at: '2026-08-25T17:00:00.000Z',
    refund_provider_reference: 'refund-ref-2',
    refund_commerce_order: 'fp-rf-bbbbbbbbbbbbbbbbbbbbbbbb',
...
const paymentIdempotencyKey = 'fran-booking-123e4567-e89b-42d3-a456-426614174000';
...
check(flowCommerceOrder === api.makeFlowCommerceOrder_(paymentIdempotencyKey)
  ...
  && /^fp-[0-9a-f]{40}$/i.test(flowCommerceOrder), 'Flow commerceOrder is deterministic and bounded to 45 chars');
check(api.makeCalendarLinkKey_(paymentIdempotencyKey).startsWith('fran-booking-calendar-link-') && !api.makeCalendarLinkKey_(paymentIdempotencyKey).includes('123e4567'), 'Calendar link identifier is opaque');
