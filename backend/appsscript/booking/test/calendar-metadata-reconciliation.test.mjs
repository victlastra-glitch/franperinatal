...
const LINK_KEY = 'fran-booking-calendar-link-metadata01';
...
function persistConfirmed(event, extra = {}) {
  ...
  return {
    reservation_id: 'fran-booking-reservation-metadata',
...
check(cancelSameTimes.outcome.changed === true && cancelSameTimes.notifications === 1
  ...
  && cancelSameTimes.outcome.refund === 'BUSINESS_POLICY_TBD',
  'cancelled/deleted event takes the clinician cancellation path and enqueues operational manual review, not a patient refund-success email');
...
const deletedError = new Error('API call to calendar.events.delete failed with error: Resource has been deleted');
check(calendar.calendarHttpStatus_(deletedError) === 404,
  'Calendar already-deleted Advanced Service errors map to 404');
const goneGateway = calendar.createCalendarGateway_({
  calendarId: 'synthetic-calendar',
  api: { Events: { remove: () => { throw deletedError; } } },
});
const gone = goneGateway.cancelLinkedEvent({ calendar_event_id: 'already-deleted-event' });
check(gone.ok === true && gone.alreadyAbsent === true,
  'cancelLinkedEvent treats already-deleted Calendar resources as absent');

let createGets = 0;
const createGateway = calendar.createCalendarGateway_({
  calendarId: 'synthetic-calendar',
  requestMeet: true,
  api: {
    Events: {
      list: () => ({ items: [] }),
      insert: (resource) => ({
        id: 'event-create-refresh', etag: 'etag-insert', updated: '2026-09-03T15:00:00.000Z',
      ...
      get: () => {
        createGets += 1;
        return {
          id: 'event-create-refresh', etag: 'etag-after-meet', updated: '2026-09-03T15:00:01.000Z',
          status: 'confirmed', start: { dateTime: START_Z }, end: { dateTime: END_Z },
          extendedProperties: { private: { source: 'fran_booking', link_key: LINK_KEY, schema: 'fran_booking:v1' } },
          conferenceData: availableMeet,
        };
      },
    },
  },
});
const created = createGateway.createLinkedBookingEvent({
  calendar_link_key: LINK_KEY, current_start_at: START_Z, current_end_at: END_Z,
});
check(createGets === 1 && created.etag === 'etag-after-meet' && created.meetStatus === 'available',
  'createLinkedBookingEvent persists the post-Meet Calendar etag');

let staleIfMatch = '';
const staleGateway = calendar.createCalendarGateway_({
  calendarId: 'synthetic-calendar',
  requestMeet: true,
  api: {
    Events: {
      get: () => ({
        id: EVENT_ID, etag: 'etag-live-meet', updated: '2026-09-03T15:00:02.000Z',
        status: 'confirmed', start: { dateTime: START_Z }, end: { dateTime: END_Z },
        extendedProperties: { private: { source: 'fran_booking', link_key: LINK_KEY, schema: 'fran_booking:v1' } },
        conferenceData: availableMeet,
      }),
      update: (_resource, _calendarId, _eventId, _args, headers) => {
        staleIfMatch = headers && headers['If-Match'];
        return {
          id: EVENT_ID, etag: 'etag-after-move', updated: '2026-09-03T15:01:00.000Z',
          status: 'confirmed', start: { dateTime: '2026-09-03T21:00:00.000Z' }, end: { dateTime: '2026-09-03T22:00:00.000Z' },
          extendedProperties: { private: { source: 'fran_booking', link_key: LINK_KEY, schema: 'fran_booking:v1' } },
          conferenceData: availableMeet,
        };
      },
...
const staleMoved = staleGateway.updateSameEvent({
  calendar_event_id: EVENT_ID, calendar_event_etag: 'etag-insert', calendar_link_key: LINK_KEY,
  current_start_at: START_Z, current_end_at: END_Z,
}, '2026-09-03T21:00:00.000Z', '2026-09-03T22:00:00.000Z');
check(staleIfMatch === 'etag-live-meet' && staleMoved.etag === 'etag-after-move',
  'same-event reschedule uses the live etag when only Meet metadata drifted');
