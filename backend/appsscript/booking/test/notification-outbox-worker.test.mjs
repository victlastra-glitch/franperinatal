...
const context = {
  ...
  GmailApp: {
    sendEmail: (to, subject, body, options) => {
      mailCalls += 1;
      mailed.push({ to, subject, body, htmlBody: options && options.htmlBody, name: options && options.name });
      return true;
    },
  },
...
function makeRecord(overrides = {}) {
  const reservationId = overrides.reservation_id || 'fran-booking-reservation-outbox';
  ...
  const base = {
    ...
    idempotency_key: 'fran-booking-outbox-key-001',
...
for (let i = 0; i < 12; i += 1) {
  many.push(makeRecord({
    reservation_id: 'fran-booking-reservation-b' + String(i).padStart(2, '0'),
    notification_outbox_key: 'lifecycle_fran-booking-reservation-b' + String(i).padStart(2, '0') + '_BOOKING_CONFIRMED_1',
  }));
}
mailed = []; mailCalls = 0;
...
mailed = []; mailCalls = 0;
const allowedRecord = makeRecord({ reservation_id: 'fran-booking-reservation-allow' });
const allowResult = runWorker(makeStore([allowedRecord]));
check(allowResult.ok && allowResult.results[0].ok && mailCalls === 1 && mailed[0].to === allowlisted, 'valid patient recipient accepted');
mailed = []; mailCalls = 0;
const rejectedRecord = makeRecord({
  reservation_id: 'fran-booking-reservation-reject',
  patient_email: 'qa+nonprod@example.test',
});
...
check(rejectResult.ok && !rejectResult.results[0].ok && mailCalls === 0 && rejectedRecord.notification_patient_state === 'failed',
  'leftover +nonprod recipient rejected without send');

// 5 + 6 + 7. CTA matrix and count=1 never RESCHEDULE
mailed = [];
const confirmed = makeRecord({ reservation_id: 'fran-booking-reservation-cta0' });
...
const rescheduled = makeRecord({
  reservation_id: 'fran-booking-reservation-cta1',
...
const countOneConfirmed = makeRecord({
  reservation_id: 'fran-booking-reservation-count1',
  patient_reschedule_count: '1',
...
mailed = [];
const persistRecord = makeRecord({ reservation_id: 'fran-booking-reservation-persist' });
...
const rotateRecord = makeRecord({ reservation_id: 'fran-booking-reservation-rotate' });
...
mailed = []; mailCalls = 0;
const sentRecord = makeRecord({ reservation_id: 'fran-booking-reservation-sent' });
...
assert.throws(() => runWorker(makeStore([makeRecord({ reservation_id: 'fran-booking-reservation-lock' })]), {
...
const maxRecord = makeRecord({
  reservation_id: 'fran-booking-reservation-max',
...
const badRecord = makeRecord({
  reservation_id: 'fran-booking-reservation-bad',
  notification_outbox_key: 'lifecycle_fran-booking-reservation-bad_NOT_A_REAL_EVENT_1',
});
const badRun = runWorker(makeStore([badRecord]), {
  outboxEntries: [durableFromRecord(badRecord, {
    eventType: 'NOT_A_REAL_EVENT',
    logical_key: 'lifecycle_fran-booking-reservation-bad_NOT_A_REAL_EVENT_1',
  })],
});
...
const mixedBad = makeRecord({
  reservation_id: 'fran-booking-reservation-mix-bad',
  patient_email: 'qa+nonprod@example.test',
});
const mixedGood = makeRecord({ reservation_id: 'fran-booking-reservation-mix-good' });
...
const sequential = makeRecord({ reservation_id: 'fran-booking-reservation-seq' });
...
runWorker(sequentialStore, { outboxStore: sequentialOutbox });
check(mailed.length === 1 && mailed[0].subject.startsWith('Tu sesión fue reagendada')
...
mailed = [];
runWorker(sequentialStore, { outboxStore: sequentialOutbox });
check(mailed.length === 1 && mailed[0].subject === 'Hubo un cambio en tu próxima sesión'
  && mailed[0].body.includes('Cancelar:') && !mailed[0].body.includes('Reagendar:'),
...
const replayBooking = makeRecord({ reservation_id: 'fran-booking-reservation-replay' });
...
const snapBooking = makeRecord({
  reservation_id: 'fran-booking-reservation-snap',
...
const snapOutbox = worker.memoryNotificationOutboxStore_([
  durableFromRecord(snapBooking, {
    eventType: 'PATIENT_RESCHEDULED',
    notification_version: '2',
    logical_key: 'lifecycle_fran-booking-reservation-snap_PATIENT_RESCHEDULED_2',
  ...
  durableFromRecord(snapBooking, {
    ...
    notification_version: '3',
    logical_key: 'lifecycle_fran-booking-reservation-snap_CLINICIAN_RESCHEDULED_3',
...
check(mailed.length === 2
  && mailed[0].subject.startsWith('Tu sesión fue reagendada')
  && mailed[1].subject === 'Hubo un cambio en tu próxima sesión'
...
const failLater = makeRecord({
  reservation_id: 'fran-booking-reservation-fail-later',
  eventType: 'PATIENT_RESCHEDULED',
  patient_reschedule_count: '1',
  reschedule_capability_revoked_at: 'used',
  notification_version: '2',
...
const maxLater = makeRecord({
  reservation_id: 'fran-booking-reservation-max-later',
  notification_patient_state: 'failed',
  notification_attempt_count: String(phase.MAX_NOTIFICATION_ATTEMPTS),
...
check(maxLaterRun.processed === 1 && maxLaterRun.results[0].ok
  ...
  && mailed.length === 1 && mailed[0].subject.startsWith('Tu sesión fue reagendada'),
  'max-attempt event stays failed and does not block a later logical send');

mailed = [];
const crash = makeRecord({ reservation_id: 'fran-booking-reservation-crash' });
...
const cancelledBooking = makeRecord({
  reservation_id: 'fran-booking-reservation-already-cancelled',
...
let clobberCalls = 0;
const raceEntry = durableFromRecord(makeRecord({ reservation_id: 'fran-booking-reservation-race' }));
...
const staleReschedule = makeRecord({
  reservation_id: 'fran-booking-reservation-stale-r',
  eventType: 'PATIENT_RESCHEDULED',
  patient_reschedule_count: '1',
  reschedule_capability_revoked_at: 'used',
});
...
const staleCancel = makeRecord({
  reservation_id: 'fran-booking-reservation-stale-c',
  booking_status: 'cancelled',
  schedule_status: 'cancelled',
  cancel_capability_revoked_at: 'now',
});
...
const clinicianBooking = makeRecord({
  reservation_id: 'fran-booking-reservation-occ-clinician',
...
typeNames.forEach((eventType) => {
  const typed = makeRecord({
    reservation_id: 'fran-booking-reservation-type-' + eventType.toLowerCase(),
    current_start_at: timeB,
    commerce_order: 'fp-1111111111111111111111111111111111111111',
    calendar_event_id: 'event-type',
    calendar_event_etag: 'etag-a',
    calendar_event_updated_at: '2026-08-25T16:00:00.000Z',
    refund_commerce_order: 'fp-rf-111111111111111111111111',
  ...
  if (eventType === 'BOOKING_CONFIRMED') {
    typed.commerce_order = 'fp-2222222222222222222222222222222222222222';
  } else {
...
const firstBatch = makeRecord({ reservation_id: 'fran-booking-reservation-lock-a' });
const secondBatch = makeRecord({ reservation_id: 'fran-booking-reservation-lock-b' });
...
const sharedEntry = durableFromRecord(makeRecord({ reservation_id: 'fran-booking-reservation-lock-race' }));
...
const liveBooking = makeRecord({ reservation_id: 'fran-booking-reservation-lock-enqueue' });
...
const maxRotate = makeRecord({
  reservation_id: 'fran-booking-reservation-max-rotate',
...
check(origin === 'https://franciscabustos.cl', 'preview origin derives from validated Preview return URL');
const sampleUrl = worker.managementPageUrl_(origin, 'a'.repeat(64), 'cancel');
check(sampleUrl.startsWith('https://franciscabustos.cl/manage.html?token=')
