...
export function assertProductionUpstreamCallSites(source) {
  const maskedSource = maskNonCode(source);
  const functions = findNamedFunctions(maskedSource);
  const calls = [];
  const callPattern = /\bproductionUpstream\s*\(\s*env\s*\)/g;
  let match;
  ...
  for (const handler of EXPECTED_UPSTREAM_HANDLERS) {
    ...
    if (handlerCalls.length !== 1) {
      throw new Error(`${handler} must contain exactly one productionUpstream(env) call; found ${handlerCalls.length}`);
    }
  }
  for (const handler of actualHandlers) {
    if (!expectedHandlers.has(handler)) {
      throw new Error(`unauthorized handler invokes productionUpstream(env): ${handler}`);
    }
  }
...
if (invokedAsScript) {
  ...
  if (!workerPath) throw new Error('usage: assert-production-worker-structure.mjs <worker-path>');
  const source = await readFile(workerPath, 'utf8');
  assertProductionUpstreamCallSites(source);
  console.log('PRODUCTION_UPSTREAM_CALL_SITES=PASS');
}
