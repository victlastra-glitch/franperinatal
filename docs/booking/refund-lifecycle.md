...
createFlowRefundGateway_ usa exclusivamente Sandbox `https://sandbox.flow.cl/api` y firma parámetros form-urlencoded. `refund/create` y `refund/cancel` usan POST; `refund/getStatus` usa GET y transporta únicamente `apiKey`, `token` y `s` como query parameters. El callback URL se limita al endpoint Preview esperado. El fake local impone estos verbos, nombres, URL y firma.
