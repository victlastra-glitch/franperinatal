...
únicamente en este proyecto. `production` es la rama canónica y por defecto: la
línea desde la que se mantiene y despliega Producción. Todo cambio va por rama,
preview y PR contra `production`. `main` es lineaje legado, preservado en
`legacy/main-pre-production-lineage-20260905`; no tiene historia en común con
`production` y no se usa para trabajo nuevo.
